...
<?php 
    ...
    $query = $conn->query("SELECT p.id,p.name,p.location,p.price,p.image,p.bed,p.bath,p.sqft,p.home_type,p.year_built,p.type,p.price_sqft,p.description,p.admin_name,p.created_at FROM props p INNER JOIN favs f ON p.id = f.prop_id WHERE f.user_id='$user_id'");
    $query->execute();
    $favProps = $query->fetchAll(PDO::FETCH_OBJ);
    $favPropsNum = $query->rowCount();
    }else{
...
?>
    ...
    <div class="container">
      ...
      <?php if($favPropsNum > 0): ?>
      <?php foreach($favProps as $prop):  ?>
      ...
      <?php else: ?>
        <div class="alert alert-danger" style="margin-left:auto;margin-right:auto;">None property has been added to Favourite</div>
      <?php endif; ?>
    </div>
